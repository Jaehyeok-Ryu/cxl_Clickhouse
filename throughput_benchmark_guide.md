...
*   **설정 파일 위치**: [config/memory_tuning.xml](file:///home/sawi/cxl_ClickHouse/config/memory_tuning.xml#L125)
...
*   **설정 파일 위치**: [config/memory_tuning.xml](file:///home/sawi/cxl_ClickHouse/config/memory_tuning.xml#L131)
...
*   **설정 파일 위치**: [config/memory_tuning.xml](file:///home/sawi/cxl_ClickHouse/config/memory_tuning.xml#L127-L129)
```xml
...
*   **서버 제한 상향 (`run_clickhouse_cluster.sh`)**:
    *   [remote_servers.xml 생성부](file:///home/sawi/cxl_ClickHouse/run_clickhouse_cluster.sh#L113): `<max_server_memory_usage>`값을 늘려줍니다. (예: 128GB로 증설 시 `137438953472`)
    *   [memory_tuning.xml 생성부](file:///home/sawi/cxl_ClickHouse/run_clickhouse_cluster.sh#L124): `<max_memory_usage>`값을 늘려줍니다. (예: 64GB로 증설 시 `68719476736`)
*   **In-Memory 조인 활성화 (`benchmark.py`)**:
    *   [benchmark.py의 쿼리 세팅부](file:///home/sawi/cxl_ClickHouse/benchmark.py#L365-L366): 디스크 스필링 임계값인 `max_bytes_before_external_group_by`와 `max_bytes_before_external_join` 설정을 더 크게 지정하여 중간 데이터를 끝까지 RAM 위에서만 처리하도록 유도합니다. (예: `34359738368` (32GB))
...
1.  **[config/memory_tuning.xml](file:///home/sawi/cxl_ClickHouse/config/memory_tuning.xml)** 수정:
    ```xml
    <clickhouse>
        <profiles>
            <default>
